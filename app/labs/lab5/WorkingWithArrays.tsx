...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <br /><br /><hr />
      <h3>Update Todo Completed and Description</h3>
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-success float-end ms-2">
        Update Completed
      </a>
      <a href={`${API}/${todo.id}/description/${todo.description}`} className="btn btn-warning float-end">
        Update Description
      </a>
      <FormControl defaultValue={todo.description} className="w-50 mt-2"
             onChange={(e) => setTodo({ ...todo, description: e.target.value }) }/>
      <FormControl type="checkbox" checked={todo.completed}
